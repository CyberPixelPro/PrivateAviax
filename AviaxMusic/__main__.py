...
async def init():
    ...
    LOGGER("Aviaxmusic.plugins").info(
        "Successfully Imported Modules "
    )
    await userbot.start()
    await Aviax.start()
    try:
        await Aviax.stream_call(
    ...
    LOGGER("AviaxMusic").info("Aviax Music Bot Started Successfully")
    await idle()


if __name__ == "__main__":
    loop.run_until_complete(init())
    LOGGER("AviaxMusic").info("Stopping Aviax Music Bot! GoodBye")
